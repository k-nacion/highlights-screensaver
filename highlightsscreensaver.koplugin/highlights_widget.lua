...
function M.buildHighlightsScreensaverWidget(clipping)
    ...
    local hs_line_height = math.min(math.max(G_reader_settings:readSetting("hs_line_height") or 0, 0), 160) / 100
    local hs_width = Screen:getWidth() * ((G_reader_settings:readSetting("hs_width_percent") or 90) / 100)
    ...
    local function buildContent(base_font_size)
        local function fontSizeAlt() return math.ceil(base_font_size * 0.75) end
        ...
        local highlight_group = HorizontalGroup:new({
            LineWidget:new({ dimen = Geom:new({ w = Size.border.thick, h = highlight_text:getSize().h }), background = col_fg }),
        ...
        local content = VerticalGroup:new({ highlight_group, VerticalSpan:new({ width = 24 }), source_text })
        ...
        if clipping.note and clipping.note ~= "" and notes_option ~= "disable" then
            ...
            table.insert(content, LineWidget:new({ dimen = Geom:new({ w = note_width, h = Size.line.thin }), background = col_fg }))
    ...
    local font_size_min  = hs_font_min
    ...
    local container = CenterContainer:new({ dimen = Screen:getSize(), content, padding = 0, margin = 0, bgcolor = col_bg })
    return ScreenSaverWidget:new({ widget = container, background = col_bg, covers_fullscreen = true })
end
